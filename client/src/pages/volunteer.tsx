...
import { Search, Filter, SlidersHorizontal, X, Heart } from "lucide-react";
...
const volunteerCategories = [
  ...
  "community", "youth", "elderly", "refugees", "other"
];

const commitmentTypes = ["full-time", "part-time", "weekends", "flexible"];
...
export default function Volunteer() {
  ...
  const { data: volunteers = [], isLoading } = useQuery<Opportunity[]>({
    queryKey: ['/api/opportunities', { type: 'volunteer', province: selectedProvince !== 'all' ? selectedProvince : undefined, search: searchQuery || undefined, status: 'approved' }],
  });
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-2">
          {volunteerCategories.slice(0, 5).map((category) => (
      ...
      <div>
        <Label className="text-sm font-medium mb-3 block">Commitment</Label>
        <div className="space-y-2">
          {commitmentTypes.map((type) => (
            <div key={type} className="flex items-center gap-2">
              <Checkbox id={`commit-${type}`} data-testid={`checkbox-commit-${type}`} />
              <Label htmlFor={`commit-${type}`} className="text-sm font-normal cursor-pointer">
                {type}
              </Label>
            </div>
          ))}
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold" data-testid="text-page-title">{t("volunteer")}</h1>
        </div>
